...
import { fetchPowerHistory } from "../services/pidMonitorApi";
...
const formatLatestUtcTimestamp = (timestampMs) => {
  if (!timestampMs) {
    return "--";
  }
...
const calculateAverageValue = (records) => {
...
const Power = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const nextPayload = await fetchPowerHistory(
          appliedRange
            ? {
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : "Failed to load power history data."
        );
      } finally {
        if (isActive) {
          setIsLoading(false);
  ...
  const channelDescription = payload?.meta?.channelDescription ?? "Engine Power";
  const latestRecord = records[records.length - 1] ?? null;
  const averageValue = useMemo(() => calculateAverageValue(records), [records]);
          ...
          <Box className="flex-1 min-h-[916px] overflow-auto rounded-[10px] bg-[#1e2939] border-[#364153] border-solid border-[1px] box-border flex flex-col items-start !p-6 max-w-full shrink-0">
            <Box className="w-full flex flex-col gap-6">
              <Box className="w-full flex flex-col gap-4">
                ...
                <Box className="flex flex-wrap items-center gap-3">
                  <Typography className="text-[#cbd5e1] font-medium">
                    Select engine:
                  </Typography>
              ...
              <Box className="grid grid-cols-1 xl:grid-cols-3 gap-4 w-full">
                <Box className="rounded-[12px] border border-[#334155] bg-[#0f172a] !p-4">
                  <Typography className="text-[12px] uppercase tracking-[0.12em] text-[#64748b]">
                    Latest Power
                  </Typography>
                  <Typography className="mt-2 text-[28px] font-semibold text-[#f8fafc]">
                    {latestRecord ? `${Number(latestRecord.value ?? 0).toFixed(2)} ${unit}` : "--"}
                  </Typography>
                </Box>
                ...
                <Box className="rounded-[12px] border border-[#334155] bg-[#0f172a] !p-4">
                  ...
                  <Typography className="mt-2 text-[28px] font-semibold text-[#8fb4ef]">
                    {averageValue != null ? `${averageValue.toFixed(2)} ${unit}` : "--"}
                  </Typography>
                </Box>
                ...
                <Box className="rounded-[12px] border border-[#334155] bg-[#0f172a] !p-4">
                  ...
                  <Typography className="mt-2 text-[20px] font-semibold text-[#e2e8f0]">
                    {formatLatestUtcTimestamp(latestRecord?.timestampMs)}
                  </Typography>
              ...
              <Box className="w-full rounded-[12px] border border-[#334155] bg-[#0f172a] !p-4">
                <TimeSeriesLineChart
                  chartData={chartData}
                  series={lineSeries}
                  rangeStartMs={payload?.meta?.rangeStartMs ?? null}
                  rangeEndMs={payload?.meta?.rangeEndMs ?? null}
                  chartHeight={360}
                  title={`${buildEngineLabel(effectiveSelectedEngine)} Load Trend`}
                  subtitle={`Time-based history for ${channelDescription} in the selected engine and UTC range.`}
                  yAxisName={unit}
                  emptyMessage={
                    isLoading
                      ? "Loading power history..."
                      : `No power history returned for ${buildEngineLabel(effectiveSelectedEngine)} in the selected window.`
                  }
                />
              </Box>
            </Box>
