...
const DeviceStatus1 = () => {
  ...
  const { payload, error, lastUpdated, pollIntervalMs } =
    usePolledPagePayload("device_status_1");
  const [monitorData, setMonitorData] = useState(null);
  const modbusConnected = error ? false : payload ? true : null;
  const svgResetKey =
    modbusConnected === false ? "device-status-1-base" : "device-status-1-live";

  useEffect(() => {
    if (!payload) {
      if (error) {
        console.error("Failed to load Device Status 1 data:", error);
        setMonitorData(null);
      }
      return;
    }
            ...
            <Box className="relative self-stretch flex-1 w-full overflow-hidden rounded-[10px] bg-transparent">
              <Box className="relative h-full w-full overflow-hidden rounded-[4px] border border-[#364153] bg-transparent">
                <img
                  ...
                  alt="Device Status 1 background"
                  src="/Device_Status_1.png"
                />
                <object
                  key={svgResetKey}
                  ref={svgObjectRef}
                  aria-label="Device Status 1 indicators overlay"
                ...
                >
                  <img
                    ...
                    alt="Device Status 1 indicators overlay"
                    src="/Indicator1.svg"
                  />
                </object>
              </Box>
