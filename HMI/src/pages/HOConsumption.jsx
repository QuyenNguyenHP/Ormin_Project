...
const HOConsumption = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const nextPayload = await fetchHOConsumptionHistory(
          appliedRange
            ? {
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : "Failed to load H.O. consumption data."
        );
      } finally {
        if (isActive) {
          setIsLoading(false);
                ...
                {engineCards.map((engineCard) => (
                  ...
                  >
                    <FOConsumptionChart
                      ...
                      chartHeight={280}
                      title={`Engine ${engineCard.engineNumber} H.O Consumption Trend`}
